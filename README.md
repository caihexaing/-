...
当前版本为 `0.4.6-date-picker-fix`（`versionCode` 28），支持 Android 8.0（API 26）及以上，目标 Android 16（API 36）。APK 包名为 `com.example.ticketassistant`。

本版本在视频流程修复基础上，进一步修复日期选择器选中日期后仍停留并最终超时的问题：若官方日历需要二次确认，将在日期已选中且确认按钮唯一时点击“确定/完成”等按钮，再等待选择器关闭并核对日期字段。另包含席别页误分类、成人票文案变体、日期整页误确认、“无座”误判无票、停用任务未停止后台服务和页面指纹计算过重修复；订单确认页会核对成人票后才发送一次提交。保留 T0～T6 脱敏诊断时间线和运行代次，仍需在用户设备和当前 12306 版本上进行真实端到端验收，不保证自动生成订单。
