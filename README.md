...
当前版本为 `0.4.4-video-flow`（`versionCode` 26），支持 Android 8.0（API 26）及以上，目标 Android 16（API 36）。APK 包名为 `com.example.ticketassistant`。

本版本针对视频流程修复首页预填站点被重复点击、无标题站点候选无法识别、日期已显示却重复打开选择器和等待诊断写入过密问题；等待不再重置动作冷却，订单确认页会核对成人票后才发送一次提交。保留 T0～T6 脱敏诊断时间线和运行代次，仍需在用户设备和当前 12306 版本上进行真实端到端验收，不保证自动生成订单。
