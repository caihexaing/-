...
开售闹钟会启动前台执行服务，重新查询并匹配目标车次和席别，然后唤起官方 App。开启无障碍服务后，App 会按已确认的页面状态辅助选择目标车次、席别和唯一同名成人乘车人，并在订单字段匹配时自动点击一次“提交订单”；识别到订单号、乘车日期、车次、席别、乘车人和待支付金额后进入待支付状态并停止。验证码、登录失效、身份核验、风控、候补和支付由用户手动处理。遇到弹窗、首页、未知页面或提交结果不明会停止并通知用户，不绕过官方安全控制。
