...
当前版本为 `0.4.7-date-single-click`（`versionCode` 29），支持 Android 8.0（API 26）及以上，目标 Android 16（API 36）。APK 包名为 `com.example.ticketassistant`。

本版本修复日期选择流程：适配当前官方 12306 的单击日期即自动生效行为，点击唯一目标日期后只等待日期选择器自动关闭并核对日期字段，不再寻找或点击“确定/完成”按钮，也不使用返回键。另包含席别页误分类、成人票文案变体、日期整页误确认、“无座”误判无票、停用任务未停止后台服务和页面指纹计算过重修复；订单确认页会核对成人票后才发送一次提交。保留 T0～T6 脱敏诊断时间线和运行代次，仍需在用户设备和当前 12306 版本上进行真实端到端验收，不保证自动生成订单。
