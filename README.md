...
当前版本为 `0.4.3-unified-state`（`versionCode` 25），支持 Android 8.0（API 26）及以上，目标 Android 16（API 36）。APK 包名为 `com.example.ticketassistant`。

本版本统一修复冷启动首页导航、站点候选重复选择、日期未确认、查询结果页与席别页误判、公告/活动文本误关闭、窗口切换和旧页面树复用、提交结果误判及人工重试卡死问题。新增 T0～T6 脱敏诊断时间线和运行代次，首页/任务配置页支持滚动。该修复仍需在用户设备和当前 12306 版本上进行真实端到端验收，不保证自动生成订单。
