...
当前版本为 `0.3.3-auto-query`（`versionCode` 15），支持 Android 8.0（API 26）及以上，目标 Android 16（API 36）。APK 包名为 `com.example.ticketassistant`。
...
开售闹钟会启动前台执行服务，重新查询并匹配目标车次和席别，然后唤起官方 App。`0.3.3-auto-query` 会使用任务中已确认的日期、出发站和到达站自动填写官方查询表单并发起查询，再定位目标车次和席别；如果官方 App 已经在查询结果页，也会先核对任务快照。随后辅助选择唯一同名成人乘车人，并在订单字段匹配时最多发送一次“提交订单”点击；识别到订单号、乘车日期、车次、席别、乘车人和待支付金额后进入待支付状态并停止。验证码、登录失效、身份核验、风控、候补和支付由用户手动处理。遇到弹窗、未知页面、页面节点为空、控件不唯一或提交结果不明会停止并通知用户，不绕过官方安全控制。任务页会显示自动查询阶段、无障碍事件、最近动作和提交锁，并可导出不含账号/证件/支付信息的诊断报告；该功能仍依赖官方 12306 当前页面结构，未经过用户设备上的真实端到端验证，不保证自动生成订单。
