...
当前版本为 `0.4.5-video-flow-fix`（`versionCode` 27），支持 Android 8.0（API 26）及以上，目标 Android 16（API 36）。APK 包名为 `com.example.ticketassistant`。

本版本在视频流程修复基础上，进一步修复席别页误分类、成人票文案变体、日期整页误确认、“无座”误判无票、停用任务未停止后台服务和页面指纹计算过重问题；等待不再重置动作冷却，订单确认页会核对成人票后才发送一次提交。保留 T0～T6 脱敏诊断时间线和运行代次，仍需在用户设备和当前 12306 版本上进行真实端到端验收，不保证自动生成订单。
